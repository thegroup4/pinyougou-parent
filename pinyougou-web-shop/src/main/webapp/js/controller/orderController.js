...
app.controller('orderController' ,function($scope,$controller,$location,orderService){
    ...
    $scope.findPage=function(page,rows){
        ...
        orderervice.findPage(page,rows).success(
    ...
    $scope.findOne=function(){
        ...
        orderService.findOne(id).success(//保存商品 orderVo  查询并回显 orderVo
            function(response){
                $scope.entity= response;

                // 调用处理富文本编辑器：
                editor.html($scope.entity.orderDesc.introduction);

                // 处理图片列表，因为图片信息保存的是JSON的字符串，让前台识别为JSON格式对象
                $scope.entity.orderDesc.itemImages = JSON.parse( $scope.entity.orderDesc.itemImages );

                // 处理扩展属性:
                $scope.entity.orderDesc.customAttributeItems = JSON.parse( $scope.entity.orderDesc.customAttributeItems );

                // 处理规格
                $scope.entity.orderDesc.specificationItems = JSON.parse($scope.entity.orderDesc.specificationItems);
    ...
    $scope.$watch("entity.order.typeTemplateId",function(newValue,oldValue){
        ...
        // 根据模板ID获得规格的列表的数据： 模板ID
        typeTemplateService.findBySpecList(newValue).success(function(response){
            $scope.specList = response;//List<Map>  map1 id text options
        });
    });
